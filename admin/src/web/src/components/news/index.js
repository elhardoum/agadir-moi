...
export default class News extends Component
{
  ...
  componentDidMount()
  {
    ...
    let cursor = this.state.cursor || getQueryArgFromSearch(this.props.location.search, 'cursor')
      , url = `/api/news${(new URLSearchParams(cursor && +cursor ? {start_at: cursor} : {})).toString().replace(/^(.)/, '?$1')}`
  ...
  switchCursor(cursor)
  {
    ...
    this.props.history.replace(`/news${(new URLSearchParams(cursor && +cursor ? {cursor: cursor} : {})).toString().replace(/^(.)/, '?$1')}`)
  }
        ...
        { items ? <div className="bg-white mt-3 p-6 shadow w-full">
          ...
          { items.length ? <div className="mb-8">
            <div className="flex items-center mb-3">
              <h3 className="flex-1 block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2">Saved News Items</h3>
              {false && <Link className="text-grey-dark mr-2 text-xs" to='/news'>Clear filters</Link>}
              <Link to='/news/new' className="bg-blue-2 no-underline focus:outline-none hover:bg-blue-light-2 px-2 py-1 rounded text-white text-xs">Add New</Link>
            </div>
          ...
          <div className="table m-auto text-sm text-grey">
            {getQueryArgFromSearch(this.props.location.search, 'cursor') ? <div>
              ...
              You don't have any news yet. <Link className="text-grey-dark" to='/news/new'>Add New &raquo;</Link>
            </div>}
            </div>
        </div></div> }
