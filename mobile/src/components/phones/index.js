...
import { View, Text, ScrollView, ActivityIndicator, Image, FlatList, Dimensions, StatusBar, Linking } from 'react-native'
...
export default class Phones extends Component
{
  ...
  loadQueryItems()
  {
    ...
    this.props.db.open().then(realm =>
    {
      ...
      this.setState({ phones: res.map(item => objPluck(item)) })
  ...
  render()
  {
    ...
    return (
      <View style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        {this.state.phones && !!this.state.phones.length && <FlatList style={{ flex: 1, backgroundColor: '#f1f1f1', paddingTop: 10 }}
          ...
          renderItem={({item, index}) => <View>
            { item.ActivityIndicator ? <ActivityIndicator size="small" color="#55d1f3" style={{ height: 50 }} /> : <View style={{
              ...
              }} onLayout={({nativeEvent: {layout: { height, width }}}) => this.setState({ [`height_${item.id}`]: width, [`width_${item.id}`]: width })}>
